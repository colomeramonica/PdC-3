    ...
    public CadastroTurma(Frame owner) {
		...
		*/
		...
		btDir.addActionListener((e)->{
			selected = listaAdicionados.getSelectedValue();
			if (!lm.isEmpty()) {
				i = lt.indexOf(selected);
					lm.add(lm.size() -1, selected);
					lt.remove(i);
				} else {
					i = lt.indexOf(selected);
					lm.add(lm.size(), selected);
					lt.remove(i);
				}	
		});
